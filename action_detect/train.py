...
class Train:
    def __init__(self, root):
        ...
        self.train_dataLoader = DataLoader(self.train_dataset, batch_size=40960, shuffle=True)

        # 加载测试数据
        self.test_dataset = PoseDataSet(root, False)
        self.test_dataLoader = DataLoader(self.test_dataset, batch_size=10240, shuffle=True)

        # 创建模型
        # self.net = NetV1()
        self.net = NetV2()
        # 加载已训练的数据
        self.net.load_state_dict(torch.load("../action_detect/checkpoint/pose.pt"))
    ...
    def __call__(self):
        for epoch in range(2000):
            ...
            for i, (imgs, tags) in enumerate(self.test_dataLoader):
                ...
                si = torch.eq(predict_targs, label_tags).sum().item()
                sum_score += si
            ...
            print(epoch, score, train_avg_loss, test_avg_loss)
            ...
            torch.save(self.net.state_dict(), f"./checkpoint/pose.pt")  # 保存训练的数据


if __name__ == '__main__':
    train = Train('/home/kls/data/human_pose')
    train()
