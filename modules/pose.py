...
class Pose:
    ...
    def __init__(self, keypoints, confidence, size):
        super().__init__()
    ...
    def get_position(self, bbox):
        ...
        x2, y2 = self.size[1]/2, self.size[0]/5 * 3
        distance = math.sqrt((x2 - x1) ** 2 + (y2 - y1) ** 2) * 0.3 + abs(y2 - y1) * 0.7
        return distance
    ...
    def draw(self, img, save_path, is_save=False, show_draw=True):
        assert self.keypoints.shape == (Pose.num_kpts, 2)
        ...
        if iw == 0 or ih == 0 or w == 0 or h == 0:
            print("error, width and height == 0")
        else:
        ...
        for part_id in range(len(BODY_PARTS_PAF_IDS) - 2):
            ...
            if global_kpt_a_id != -1 and global_kpt_b_id != -1:
                ...
                cv2.line(I, (int(px_a * scale), int(py_a * scale)), (int(px_b * scale), int(py_b * scale)),
                         [255, 255, 255], 2)
            # **************************************
        ...
        # 保存骨骼图片
        if is_save:
            cv2.imwrite(save_path + f'{t}.jpg', I)
        # **************************************
