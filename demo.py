...
def run_demo(video_source, net, action_net, image_provider, height_size, cpu, track, smooth):
    ...
    for img in image_provider:
        ...
        if i % 1 == 0:
            ...
            for pose in current_poses:
                ...
                r_sho, r_elb, r_wri = pose.keypoints[2][1], pose.keypoints[3][1], pose.keypoints[4][1]
                l_sho, l_elb, l_wri = pose.keypoints[5][1], pose.keypoints[6][1], pose.keypoints[7][1]

                if r_wri < r_elb < r_sho:
                    r_hand_raise = True
                if l_wri < l_elb < l_sho:
                    l_hand_raise = True
                ...
                c = l_wri - l_elb
                d = 2 * (l_sho - l_elb)
                if (r_wri - r_elb) > 2 * (r_sho - r_elb) > 0:
                    r_hand_raise = True
                if (l_wri - l_elb) > 2 * (l_sho - l_elb) > 0:
                    l_hand_raise = True
                ...
                if position < 450:
                    ...
                    if one_hand_raise:
                        if not os.path.exists(os.path.join(img_path, tag1)):
                            os.mkdir(os.path.join(img_path, tag1))
                        cv2.imwrite(os.path.join(img_path, tag1) + f'{str(i).zfill(6)}.jpg', img)

                    if not r_hand_raise and not l_hand_raise:
                        if not os.path.exists(os.path.join(img_path, tag0)):
                            os.mkdir(os.path.join(img_path, tag0))
                        cv2.imwrite(os.path.join(img_path, tag0) + f'{str(i).zfill(6)}.jpg', img)
    ...
    if os.path.exists(os.path.join(img_path, tag2)):
...
if __name__ == '__main__':
    detect_main(video_source=r'/mnt/marathon/staticresource/person_tracking/videos_person_tracking/姿势'
                             r'/000009_000000_FN001211_TID000032.avi',
                video_name='video1')
